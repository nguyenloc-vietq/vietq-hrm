...
// import AttendanceApi from 'src/services/api/attendance.api';
...
import { TableSkeleton } from 'src/sections/user/user-table-row';
...
const TABLE_HEAD = [
  ...
  { id: 'payrollCode', label: 'Payroll Code', width: 120 },
  { id: 'fullName', label: 'Full Name', width: 180 },
  { id: 'workDay', label: 'Work Day', width: 100 },
  { id: 'timeIn', label: 'Time In', width: 100 },
  { id: 'timeOut', label: 'Time Out', width: 100 },
  { id: 'status', label: 'Status', width: 120 },
  { id: 'lateMinutes', label: 'Late Minutes', width: 100 },
  { id: 'earlyMinutes', label: 'Early Minutes', width: 140 },
...
export function AttendanceListRecords() {
  ...
  const fetchListAttendance = async () => {
    try {
      isLoading.onTrue();
      const ListAttendance = await AttendanceApi.getAdminListAttendance();
      console.log(`[===============> List Attendance | `, ListAttendance);
      setTableData(ListAttendance);
    } catch (error) {
      console.log(error);
    } finally {
      isLoading.onFalse();
  ...
  const fetchListAttendanceByMonth = async () => {
    try {
      isLoading.onTrue();

      const ListAttendance = await AttendanceApi.getAdminListAttendance(startDate, endDate);
      console.log(`[===============> List Attendance | `, ListAttendance);
      setTableData(ListAttendance);
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.attendance.edit(id));
    },
    [router]
  );
